...
function Control(state, opts) {
  ...
  this.onForward = opts.onForward || this.onForward
  this.onBackward = opts.onBackward || this.onBackward
  this.onJump = opts.onJump || this.onJump
  this.onLeft = opts.onLeft || this.onLeft
  this.onRight = opts.onRight || this.onRight
  this.firing = 0
...
proto.tick = function(dt) {
  ...
  if(state.backward) {
    ...
    if(target.velocity.z < this.max_speed)
      target.velocity.z = max(min(this.max_speed, move_speed * dt * this.acceleration
        (this.z_accel_timer, this.accel_max_timer)), target.velocity.z)
  } else if(state.forward) {
    this.onForward();
    if(target.velocity.z > -this.max_speed)
      target.velocity.z = min(max(-this.max_speed, -move_speed * dt * this.acceleration
        (this.z_accel_timer, this.accel_max_timer)), target.velocity.z)
  } else {
  ...
  if(state.right) {
    ...
    if(target.velocity.x < this.max_speed)
      target.velocity.x = max(min(this.max_speed, move_speed * dt * this.acceleration
        (this.x_accel_timer, this.accel_max_timer)), target.velocity.x)
  } else if(state.left) {
    this.onLeft();
    if(target.velocity.x > -this.max_speed)
      target.velocity.x = min(max(-this.max_speed, -move_speed * dt * this.acceleration
        (this.x_accel_timer, this.accel_max_timer)), target.velocity.x)
  } else {
  ...
  if(state.jump) {
    if(!this.jumping && !at_rest) {
      ...
      if(this.jump_timer > 0) {
        target.velocity.y = min(target.velocity.y + jump_speed * min(dt, this.jump_timer),
          this.jump_max_speed)
      }
  ...
  if(state.fire || state.firealt) {
    if(this.firing && this.needs_discrete_fire) {
      ...
      if(!this.fire_rate || floor(this.firing / this.fire_rate) !== floor((this.firing + dt)
        / this.fire_rate)) {
        this.onfire(state)
      }
      this.firing += dt
  ...
  pitch_target.rotation.x = clamp(pitch_target.rotation.x + clamp(x_rotation,
    this.x_rotation_per_ms), this.x_rotation_clamp)
  yaw_target.rotation.y = clamp(yaw_target.rotation.y + clamp(y_rotation,
    this.y_rotation_per_ms), this.y_rotation_clamp)
  roll_target.rotation.z = clamp(roll_target.rotation.z + clamp(z_rotation,
    this.z_rotation_per_ms), this.z_rotation_clamp)
...
proto.onForward = function(){
  
}

proto.onBackward = function(){

}

proto.onLeft = function(){

}

proto.onRight = function(){

}

proto.onJump = function(){
